...
def get_class():
    ...
    for i in range(0, len(csv.index), step):
        t = Thread(target=request_class, args=(csv.iloc[i: i + step].copy() if i + step <= len(csv.index) else csv.iloc[i:].copy(), q, len(csv.index)//step, step))
        t.start()
        threads.append(t)
        # break
    for t in threads:
...
def get_keyword():
    ...
    threads = []
    for i in range(0, len(csv.index), step):
        t = Thread(target=request_keyword, args=(csv.iloc[i: i + step].copy() if i + step <= len(csv.index) else csv.iloc[i:].copy(), q, len(csv.index)//step, step))
        t.start()
        threads.append(t)
        # break
    for t in threads:
